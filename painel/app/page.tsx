        ...
        <main className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
          ...
          <section className={`p-8 rounded-[32px] border ${theme.card}`}>
            <div className="flex flex-col md:flex-row gap-8 items-center">
              <div className="flex-1 space-y-4">
                <h3 className="font-black text-lg uppercase flex items-center gap-3"><Search size={20} className="text-blue-500"/> Rastreio CNES</h3>
                <div className="flex gap-2">
                  <input type="text" placeholder="CNPJ..." className={`flex-1 px-5 py-4 rounded-2xl text-sm font-bold border outline-none ${isLightMode ? 'bg-slate-50 border-slate-200' : 'bg-black/20 border-white/5'}`} value={buscaCnpj} onChange={(e) => setBuscaCnpj(e.target.value)} />
                  <button onClick={handleSearchCnpj} disabled={searchingApi} className={`px-8 py-4 rounded-2xl font-black text-xs uppercase ${theme.accent}`}>{searchingApi ? "..." : "Rastrear"}</button>
                </div>
              </div>
              <AnimatePresence>
                {unidadeReal && (
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pb-10">
              {evidencias.map((doc: any, idx: number) => (
